...
const ChooseOrder = ({ orders = [], onAddOrder, onUpdate, onDelete }) => {
  ...
  const handleAddSubmit = (e) => {
    ...
    const orderData = {
      ...
      totalAmount: newOrder.items.reduce(
  ...
  const handleUpdateSubmit = (e) => {
    ...
    const orderData = {
      ...
      totalAmount: editOrder.items.reduce(
        (sum, item) => sum + calculateEditSubtotal(item),
        0
      ),
    ...
    setEditOrder({
      ...
      items: editOrder.items.map((item) => {
        if (item.id === itemId) {
          ...
          // Nếu thay đổi product_id, xóa product object cũ
          if (field === "product_id") {
  ...
  const calculateEditSubtotal = (item) => {
    ...
    const product = products.find(
      (p) => p.id === parseInt(item.product_id || item.product?.id)
    );
  ...
  return (
    <>
          ...
          {filteredOrders.length > 0 ? (
            filteredOrders.map((value) => (
              <tr key={value.id}>
                ...
                <td>
                  <span
                    className={`status-badge status-${value.status?.toLowerCase()}`}
                  >
                    {value.status}
                  </span>
                </td>
          ...
          <div className="modal" style={{ maxWidth: "700px" }}>
            ...
            <form onSubmit={handleAddSubmit}>
              ...
              <div style={{ marginTop: "20px" }}>
                ...
                >
                  <label style={{ margin: 0 }}>List of dishes</label>
                  <button
                    type="button"
                ...
                {newOrder.items.length > 0 ? (
                  ...
                  >
                    There are no items yet. Click "Add item" to add.
                  </p>
                )}
              </div>
      ...
      {showEditForm && (
        <div className="modal-overlay">
          <div className="modal" style={{ maxWidth: "700px" }}>
            <h2>Update orders {editOrder.id}</h2>
                          ...
                          >
                            ...
                            <div>
                              ...
                              <input
                                type="text"
                                value={
                                  calculateEditSubtotal(item).toLocaleString(
                                    "vi-VN"
                                  ) + "₫"
                                }
                                disabled
          ...
          <div className="order-details-sidebar">
            ...
            <div className="order-info-section">
              <div className="info-row">
                ...
                <span className="info-value">
                  Table {editOrder.table?.id || "—"}
                </span>
              </div>
              <div className="info-row">
                <span className="info-label">Status</span>
                <span
                  className={`status-badge status-${editOrder.status?.toLowerCase()}`}
                >
                  {editOrder.status}
                </span>
              </div>
              <div className="info-row">
                ...
                <span className="info-value">
                  {editOrder.createdAt
                    ? new Date(editOrder.createdAt).toLocaleString("vi-VN")
                    : "—"}
                </span>
              </div>
            </div>
            ...
            <div className="order-items-section">
              <h3 className="section-title">
                🍽️ List of dishes ({orderItems.length})
              </h3>
                      ...
                      <div className="item-header">
                        ...
                        <div className="item-price">
                          <p className="item-unit-price">
                            {item.product?.price?.toLocaleString("vi-VN")}₫/món
                          </p>
              ...
              <div className="total-section">
                <div className="total-row">
                  ...
                  <span className="total-amount">
                    {editOrder.total_amount?.toLocaleString("vi-VN")}₫
                  </span>
                </div>
              </div>
            </div>
